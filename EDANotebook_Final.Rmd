...
```{r}
# Read the csv file
nanda <- read.csv('C:/Users/fhasin/Downloads/Customer Contracts/nanda data/csv file.csv', colClasses = c(tract_fips10 = "character"))
```
...
ggplot(my_data_bystate, aes(x = year, y = count_sales_7225)) + geom_boxplot()
ggplot(my_data_bystate %>% filter(year == 2007), aes(x = statecol, y = count_sales_7225)) + geom_boxplot() + ggtitle("Distribution of Restaurants with Sales >0 in 2010 Census Tracts by State (2007)")
```
...
The code is creating a scatter plot with a linear regression line. This scatter plot shows the relationship between 'popden_722511', which is the population density, and 'count_722511', which is the number of full-service restaurants. 'geom_point()' is used for the data points and 'geom_smooth(method = "lm")' is used to generate the linear trend. There are additionally labels generated to make it easier to interpret the graph. This scatter plot is used to identify trends to see if areas with higher populations have more full-service restaurants.  

```{r}
# Exploring the relationship between population density and the number of full-service restaurants
my_data_bystate %>%
  ggplot(aes(x = popden_722511, y = count_722511)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Relationship between Population Density and Number of Full-Service Restaurants",
       x = "Population Density", y = "Count of Full-Service Restaurants")
```

The code first generates a summary of the variable 'popden_722511' which represents the population density by state since it is from the 'my_data_bystate' dataset. This summary will allow to see any outliers and provide an insight into general factors such as mean and median. 

Then the outliers are identified. Instances when 'popden_722511' variable exceeds 500,000 is filtered out since it is considered unrealistic. 

After the outlier is identified, the my_data_bystate is filter the extreme values of population density.

Lastly, a scatterplot is generated with a regression line to see the relationship between the population density, 'popden_722511'. and number of full-service density, 'count_722511'. This is a more refined version that shows more realistic conditions without the influence of outliers. This allows for more accuracy.

```{r}
summary(my_data_bystate$popden_722511)
outlier <- my_data_bystate %>% filter(popden_722511 >= 500000)
print(outlier)
#This must be an error in the data - Laramie, WY does not have an astronomically high population density
my_data_bystate <- my_data_bystate %>% filter(popden_722511 < 500000)
my_data_bystate %>%
  ggplot(aes(x = popden_722511, y = count_722511)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
A correlation matrix is generated to explore the relationships between the numerical values within the dataset, 'my_data_bystate'.

The numeric columns is selected from the dataset by using 'sapply(my_data_bystate, is.numeric). This creates a new dataset of 'numerical_data'. This steps make sure the correlation analysis only has variables that are numerical.

The 'cor()' function calculates the correlation coefficients of the numerical values within 'numerical_data'. This is done through using the method 'pairwise.complete.obs'. This generates the matrix 'correlation_matrix'. 

Then, the 'corrplot()" function is used to visualize the correlation matrix. The method used is circles to indiate strenght of each correlation coefficient.
```{r}

# Correlation Matrix

numerical_data <- my_data_bystate[sapply(my_data_bystate, is.numeric)] 
...
A density plot for 'population' variable from 'my_data_bystate' dataset is generated. This is generated to provide a visual on how population density is distributed across states.

```{r}
# Density Plot of Population Density
ggplot(my_data_bystate, aes(x = population)) + 
  geom_density(fill="blue", alpha=0.5) +
  labs(title = "Density Plot of Population Density",
       x = "Population Density", y = "Density") +
  theme_minimal()
```
...
The scatterplot compares 'popden_7225', population density, to 'count_7225', establishment count. The points are color coded by 'year'. This allows for the relationship to be visiualized while still visualizing by year. The 'facter_wrap(~year)' function seperates the scatterplot for each year. This allows for direct comparisons of the visualizations.
 
```{r}
# Faceted Scatterplot for Population Density and Establishment Count by Year
ggplot(my_data_bystate, aes(x = popden_7225, y = count_7225)) +
  geom_point(aes(color = year)) +
  facet_wrap(~year) +
  labs(title = "Population Density vs Establishment Count by Year",
       x = "Population Density", y = "Establishment Count") +
  theme_minimal()
```

This code visualizes the trend of 'count_sales_7225', establishment sales, over various years. The data points are connected by a blue line using 'geom_line'. Plot titles and 'theme_minimal()' is applied to make the data appear more organized. 

```{r}
# Time Series of Establishment Sales over the Years
ggplot(my_data_bystate, aes(x = year, y = count_sales_7225)) + 
  geom_line(group=1, colour="blue") +
  labs(title = "Time Series of Establishment Sales over the Years",
       x = "Year", y = "Total Count") +
  theme_minimal()
```
...
```{r}
# Summarize establishment counts by state
establishment_counts_by_state <- my_data_bystate %>%
  group_by(statecol) %>%
...
# Plot
ggplot(establishment_counts_by_state, aes(x = reorder(statecol, -count), y = count, fill = establishment_type)) + 
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip() +
  labs(title = "Establishment Counts by State",
       x = "State", y = "Count") +
  theme_minimal() +
...
ggplot(heatmap_data_melted, aes(x = establishment_type, y = statecol, fill = count)) + 
  geom_tile() + 
  labs(title = "Heatmap of Establishment Types by State",
...
```{r}
#Comparing the density of establishments across states can reveal geographical trends and hotspots:
my_data_long %>%
  group_by(statecol, establishment_type) %>%
  ...
  labs(title = "Average Establishment Count by State and Type",
       x = "State",
       y = "Average Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
obesity_data <- read.csv('C:/Users/fhasin/Downloads/Customer Contracts/nanda data/CDC Obesity 2022-overall-prevalence.csv')
```
